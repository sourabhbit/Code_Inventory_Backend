  ...
  switch (showBy) {
    case "category":
      ...
      if (itemName) {
        ...
        query =
          "SELECT * FROM items WHERE category_id = ? AND user_id = ? ORDER BY name ASC";
      }
    ...
    case "uncategorized":
      if (itemName) {
        ...
        query =
          "SELECT * FROM items WHERE category_id IS NULL AND user_id = ? ORDER BY name ASC";
      }
...
itemsController.createItem = (req, res) => {
  ...
  categoryId =
    !categoryId || categoryId === NaN || categoryId === "NULL"
      ? null
      : parseInt(categoryId);
  ...
  itemNameExists(categoryId, req.userId, name, (exists) => {
    if (!exists) {
      const query =
        "INSERT INTO items (category_id, user_id, name, quantity, unit, image_url) VALUES (?, ?, ?, ?, ?, ?)";
      mysqlConnection.query(
        query,
        [categoryId, req.userId, name, quantity, unit, imageURL],
        (error, rows, fields) => {
          !error
            ? res.status(200).send({ status: "Item added", id: rows.insertId })
            : console.log(error);
        }
      );
    } else {
...
itemsController.updateItem = (req, res) => {
  ...
  categoryId =
    categoryId === "NaN" || categoryId === "NULL" ? null : categoryId;
  let query = "";
  if (nameIsEquals) {
    query =
      "UPDATE items SET category_id = ?, quantity = ?, unit = ? WHERE id = ? AND user_id = ?";
    mysqlConnection.query(
      query,
      [categoryId, quantity, unit, id, req.userId],
      (error, rows, fields) => {
        if (!error) {
    ...
    itemNameExists(categoryId, req.userId, name, (exists) => {
      if (!exists) {
        query =
          "UPDATE items SET category_id = ?, name = ?, quantity = ?, unit = ? WHERE id = ? AND user_id = ?";
        mysqlConnection.query(
          query,
          [categoryId, name, quantity, unit, id, req.userId],
          (error, rows, fields) => {
            if (!error) {
              if (rows.changedRows > 0) {
                res.status(200).send({ status: "Item updated" });
...
const itemNameExists = (categoryId, userId, name, exists) => {
  ...
  if (!categoryId) {
    query =
      "SELECT * FROM items WHERE category_id IS NULL AND user_id = ? AND name = ?";
    ...
    query =
      "SELECT * FROM items WHERE category_id = ? AND user_id = ? AND name = ?";
    mysqlConnection.query(
      query,
